...
def test_array(STREAM_ARRAY_SIZE):
    a_array = array.array('f', [])
    b_array = array.array('f', [])
    c_array = array.array('f', [])
    for j in range(STREAM_ARRAY_SIZE):
        a_array.append(1.0)
        b_array.append(2.0)
        c_array.append(0.0)
    ...
    times[1] = timer()
    for j in range(STREAM_ARRAY_SIZE):
        b_array[j] = scalar * c_array[j]
    times[1] = timer() - times[1]
    # sum
    times[2] = timer()
    for j in range(STREAM_ARRAY_SIZE):
        c_array[j] = a_array[j] + b_array[j]
    times[2] = timer() - times[2]
...
def test_numpy(STREAM_ARRAY_SIZE):
    ...
    times[1] = timer()
    for j in range(STREAM_ARRAY_SIZE):
        b_np[j] = scalar * c_np[j]
    times[1] = timer() - times[1]
    # sum
    times[2] = timer()
    for j in range(STREAM_ARRAY_SIZE):
        c_np[j] = a_np[j] + b_np[j]
    times[2] = timer() - times[2]

    # triad
    times[3] = timer()
    for j in range(STREAM_ARRAY_SIZE):
        a_np[j] = b_np[j] + scalar * c_np[j]
    times[3] = timer() - times[3]
...
if __name__ == "__main__":
    STREAM_ARRAY_SIZE = np.logspace(0, 9, 10, base=2,dtype=int)
    ...
    record_np = record_np.reshape(10,4)
    record_arr = record_arr.reshape(10, 4)
    record_list = record_list.reshape(10, 4)
    # drew figures
